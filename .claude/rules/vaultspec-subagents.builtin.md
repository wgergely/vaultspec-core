---
name: vaultspec-subagents.builtin
trigger: always_on
---

# Sub-Agents

Sub-agents are the de facto standard for performing any meaningful work. They are invoked using the `vaultspec-subagent` skill.

## Mechanism

This is the **utility skill** that powers the project's agentic workflows.

* **Primary Usage:** You will be instructed to use this skill by high-level workflow skills (e.g., `vaultspec-execute` will tell you to "Invoke `vaultspec-subagent` with `vaultspec-complex-executor`").
* **Direct Usage:** You should only invoke this skill directly if the user explicitly requests a specific, isolated agent dispatch (e.g., "Dispatch the vaultspec-code-reviewer to check file X") that falls outside the standard Research -> Plan -> Execute workflow.
