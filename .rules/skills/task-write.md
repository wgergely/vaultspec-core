---
description: "Use this skill to write implementation plans, task flows. It must be explicitly called after a task-adr skill has yielded an approved ADR document."
---

# Task Write Skill (task-write)
...
This skill MUST be called after `task-adr` concludes with architectural approval. Do NOT use for trivial tasks.

## Important

- If part of the `task-research` -> `task-adr` flow, this skill **MUST** be provided with the relevant `<Research>` and `<ADR>` documents.
...
- **Persistence:**
  - `<Plan>`s: `.docs/plan/yyyy-mm-dd-<feature>-<phase>-plan.md`
  - `<Phase Summary>`s: `.docs/exec/yyyy-mm-dd-<feature>/yyyy-mm-dd-<feature>-<phase>-summary.md`
  - `<Step Record>`s: `.docs/exec/yyyy-mm-dd-<feature>/yyyy-mm-dd-<feature>-<phase>-<step>.md`

## Template

- You MUST read and use the template at `.rules/templates/PLAN.md`.
...
1.  **`tags`**: MUST contain **EXACTLY TWO** tags in a YAML list.
    ...
    *   *Syntax:* `tags: ["#plan", "#feature"]` (Must be quoted strings in a list).
...
- **Drafting**: Invoke the `task-subagent` skill with `task-writer`. Instruct it to "Create an implementation plan for [feature] based on [[...-adr.md]]. Use the template at .rules/templates/PLAN.md."
- **Review**: Present the saved `<Plan>` to the user.
- **Provide an absolute link** `.docs/plan/yyyy-mm-dd-<feature>-<phase>-plan.md` **and prompt user**:
...
- **Approval Loop**: User must explicitly approve the `<Plan>`. If changes are requested, invoke the `task-subagent` skill with `task-writer`. Instruct it to "Revise the plan based on user feedback: [feedback]."
