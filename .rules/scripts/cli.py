...
def init_paths(root: Path) -> None:
    ...
    SYSTEM_SRC_DIR = root / ".rules" / "system"
    FRAMEWORK_CONFIG_SRC = root / ".rules" / "FRAMEWORK.md"
    PROJECT_CONFIG_SRC = root / ".rules" / "PROJECT.md"

    # Backward compatibility: warn if old filenames still exist
    old_internal = root / ".rules" / "INTERNAL.md"
    old_custom = root / ".rules" / "CUSTOM.md"
    if old_internal.exists() and not FRAMEWORK_CONFIG_SRC.exists():
        print(
            "Warning: .rules/INTERNAL.md is deprecated. Rename to .rules/FRAMEWORK.md",
            file=sys.stderr,
        )
    if old_custom.exists() and not PROJECT_CONFIG_SRC.exists():
        print(
            "Warning: .rules/CUSTOM.md is deprecated. Rename to .rules/PROJECT.md",
            file=sys.stderr,
        )
...
def config_sync(args: argparse.Namespace) -> None:
    ...
    if not FRAMEWORK_CONFIG_SRC.exists():
        print("  Error: No framework config found at .rules/FRAMEWORK.md")
        return
    ...
    for _tool_name, cfg in TOOL_CONFIGS.items():
        ...
        if (
            ...
            msg = "           Migrate to .rules/PROJECT.md, then --force."
            print(msg)
            result.skipped += 1
            continue
...
def collect_system_parts() -> dict[str, tuple[Path, dict[str, Any], str]]:
    """Collect system prompt parts from .rules/system/."""
    sources: dict[str, tuple[Path, dict[str, Any], str]] = {}
...
def _collect_agent_listing() -> str:
    ...
    lines = [
        ...
        "",
        "Sub-agents are specialized expert agents available in `.rules/agents/`.",
        "Dispatch using the `task-subagent` skill.",
        "",
    ]
...
def _generate_system_prompt(cfg: ToolConfig) -> str | None:
    """Assemble system prompt for a tool from .rules/system/ parts."""
...
def system_show(_args: argparse.Namespace) -> None:
    ...
    parts = collect_system_parts()
    if not parts:
        print("No system parts found in .rules/system/")
        return
