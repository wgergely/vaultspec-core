...
try:
    ...
    def _yaml_load(text: str) -> dict:
        """Minimal 2-level YAML parser (flat keys + one level of indented maps)."""
        data: dict = {}
    ...
    def _yaml_dump(data: dict) -> str:
        """Minimal YAML serializer for flat + 1-level nested dicts."""
        lines: list[str] = []
...
CONFIG_HEADER = "<!-- AUTO-GENERATED by cli.py config sync. Do not edit manually. -->"
...
def init_paths(root: Path) -> None:
    ...
    Called once at module load with the default repo root.
...
# Default initialisation — production behaviour unchanged
...
def build_file(frontmatter: dict, body: str) -> str:
...
def collect_rules() -> Dict[str, Tuple[Path, dict, str]]:
    ...
    for src_dir, label in [(RULES_SRC_DIR, "built-in"), (RULES_CUSTOM_DIR, "custom")]:
...
def transform_rule(tool: str, name: str, meta: dict, body: str) -> str:
...
def rules_add(args: argparse.Namespace) -> None:
    ...
    if file_path.exists() and not args.force:
        print(f"Error: Rule '{file_name}' already exists. Use --force to overwrite.")
        return
...
def transform_agent(tool: str, name: str, meta: dict, body: str) -> Optional[str]:
...
def agents_list(args: argparse.Namespace) -> None:
    ...
    print("-" * 83)
    for name, (path, meta, body) in sources.items():
        agent_name = Path(name).stem
        tier = meta.get("tier", "?")
...
def agents_add(args: argparse.Namespace) -> None:
    ...
    if file_path.exists() and not getattr(args, "force", False):
        print(f"Error: Agent '{file_name}' already exists. Use --force to overwrite.")
        return
...
def agents_set_tier(args: argparse.Namespace) -> None:
    ...
    if new_tier not in ("LOW", "MEDIUM", "HIGH"):
        print(f"Error: Invalid tier '{new_tier}'. Must be LOW, MEDIUM, or HIGH.")
        return
...
def collect_skills() -> Dict[str, Tuple[Path, dict, str]]:
    ...
    for f in sorted(SKILLS_SRC_DIR.glob("task-*.md")):
        content = f.read_text(encoding="utf-8")
        meta, body = parse_frontmatter(content)
        # Key is the skill name (stem), not filename
        sources[f.stem] = (f, meta, body)
    return sources


def transform_skill(tool: str, name: str, meta: dict, body: str) -> str:
...
def skills_add(args: argparse.Namespace) -> None:
    ...
    if file_path.exists() and not getattr(args, "force", False):
        print(f"Error: Skill '{file_name}' already exists. Use --force to overwrite.")
        return
...
def sync_files(
    ...
    for name, (src_path, meta, body) in sources.items():
...
def sync_skills(
    ...
    """Sync skill files to a destination skills directory (handles subdirs)."""
    result = SyncResult()
    ...
    # Prune: only prune task-* prefixed skill dirs, protect others
    if prune:
        source_names = set(sources.keys())
...
def _collect_rule_refs(cfg: ToolConfig) -> List[str]:
    ...
    for rule_file in sorted(cfg.rules_dir.glob("*.md")):
        try:
            ...
            refs.append(str(rule_file.relative_to(ROOT_DIR)).replace("\\", "/"))
    return refs
...
def config_sync(args: argparse.Namespace) -> None:
    """Sync INTERNAL.md and CUSTOM.md to tool configs."""
    dry_run = getattr(args, "dry_run", False)
    ...
    for tool_name, cfg in TOOL_CONFIGS.items():
        ...
        if (
            ...
            msg = (
                "           Migrate content to .rules/CUSTOM.md, "
...
def main() -> None:
    ...
    config_sub.add_parser(
        "show", help="Display current internal/custom content and generated references"
    )

    config_sync_parser = config_sub.add_parser(
        "sync", help="Sync internal and custom configs to tool configs"
    )
