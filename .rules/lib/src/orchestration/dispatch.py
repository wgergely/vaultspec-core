...
from acp.schema import ClientCapabilities, FileSystemCapability, Implementation, TextContentBlock

from ..protocol.acp.client import DispatchClient, SessionLogger
from ..protocol.acp.types import DispatchError, DispatchResult, SecurityError
...
from .utils import parse_frontmatter, safe_read_text
...
def load_agent(agent_name: str, root_dir: pathlib.Path, provider_name: str | None = None) -> Tuple[Dict[str, str], str]:
...
def get_provider_for_model(model_name: Optional[str]) -> AgentProvider:
    ...
    # TODO: investigate - Fallback to Gemini for unknown models for now
    return gemini
...
async def _interactive_loop(
    ...
    while True:
        ...
        if not interactive:
            if debug:
                logger.debug(f"Turn finished ({response.stop_reason}). Exiting one-shot task.")
            break
        
        # Check if process is still alive
        if proc.returncode is not None:
            if debug:
                logger.debug(f"Agent process exited with code {proc.returncode}. Exiting loop.")
            break
...
async def run_dispatch(
    ...
    meta, persona = load_agent(agent_name, root_dir, provider_name=initial_provider_hint)
    
    # Determine initial model
    if not current_model:
        current_model = meta.get("model", "gemini-2.5-flash") # TODO: investigate - Default
    
    gemini = GeminiProvider() # TODO: investigate - Default/Primary provider
    ...
    while True:
        ...
        if provider_override:
            ...
            if is_mismatch:
                 if debug: logger.debug(f"Provider mismatch detected (Provider: {provider.name}, Model: {current_model}). resolving equivalent...")
                 ...
                 if debug: logger.debug(f"Resolved equivalent model: {current_model}")
        ...
        try:
            ...
            if debug:
                 logger.debug(f"Spawning agent process: {spec.executable} {' '.join(spec.args)}")
            
            client = client_class(root_dir=root_dir, debug=debug, quiet=quiet, mode=mode)
                ...
                if proc.stderr:
                    try:
                        while True:
                            ...
                            if debug:
                                try:
                                    logger.debug(f"[AGENT-STDERR] {line.decode().strip()}")
                                except Exception:
                                    pass
                ...
                *spec.args,
                ...
                try:
                    ...
                    if _SESSION_RESUME_ENABLED and resume_session_id:
                        try:
                            session = await conn.load_session(session_id=resume_session_id)
                            if debug: logger.debug(f"Resumed session: {session.session_id}")
                        except Exception as exc:
                            if debug: logger.debug(f"Session resume failed ({exc}), creating new session")
                    ...
                    if debug: logger.debug(f"Session started: {session.session_id}")

                    session_logger = SessionLogger(session.session_id, root_dir)
                    client.set_logger(session_logger)
                    if debug: logger.debug(f"Logging to: {session_logger.log_file}")

                    # Interactive Loop
                    start_prompt = getattr(spec, "initial_prompt_override", None) or initial_task
                    
                    await _interactive_loop(
                        conn, session.session_id, agent_name, start_prompt, debug, interactive, _proc, session_logger
                    )
                    
                    if debug: logger.debug("Interaction loop finished. Shutting down...")
            ...
            if debug: logger.debug("Task completed.")
