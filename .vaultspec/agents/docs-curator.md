...
- **Dispatch Sub-Agent:**
    Invoke the `spec-subagent` skill with `simple-executor`. Instruct it to "Execute the following curation task (ensure strict file naming and frontmatter compliance): [Your detailed instruction]."
...
- **`spec-subagent`**: Use for ALL modifications.
