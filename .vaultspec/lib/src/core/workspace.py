...
def _validate(layout: WorkspaceLayout) -> None:
    ...
    if not lib_dir.is_dir():
        raise WorkspaceError(
            f"framework_root/lib does not exist: {lib_dir}\n"
            f"The framework Python code was not found at the expected "
            f"location. This usually means the structural bootstrap in "
            f"_paths.py is broken."
        )
