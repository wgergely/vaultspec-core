...
class TestAgentCardFromDefinition:
    def test_creates_card_with_name(self):
        meta = {"description": "Test agent", "name": "researcher", "tags": ["test"]}
        card = agent_card_from_definition(
            "researcher", meta, host="localhost", port=10099
        )
    ...
    def test_default_description(self):
        ...
        assert "researcher" in card.description

    def test_has_one_skill(self):
        meta = {"description": "Test", "name": "researcher", "tags": ["test"]}
        card = agent_card_from_definition(
            "researcher", meta, host="localhost", port=10099
        )
    ...
    def test_uses_default_config_for_host_port(self):
        meta = {"description": "Test"}
        card = agent_card_from_definition("researcher", meta)
