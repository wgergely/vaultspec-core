...
from .conftest import AsyncIteratorMock, make_sdk_mock
...
class TestPrompt:
    ...
    async def test_raises_without_session(
        self, _mock_options_cls, _mock_sdk_cls, bridge
    ):
    ...
    async def test_error_result_sets_refusal(
        ...
        error_msg.result = "Error occurred"

        mock_sdk_cls.return_value = make_sdk_mock(messages=[error_msg])
    ...
    async def test_exception_sets_refusal(
        ...
        mock_client.receive_messages = MagicMock(
            return_value=AsyncIteratorMock([], raise_exc=RuntimeError("stream failed"))
        )
        mock_sdk_cls.return_value = mock_client
...
class TestEmitUpdates:
    ...
    @pytest.mark.asyncio
    async def test_routes_assistant_message(self, connected_bridge, mock_conn):
    ...
    @pytest.mark.asyncio
    async def test_routes_user_message(self, connected_bridge, mock_conn):
        """UserMessage is routed to _emit_user_message."""
        from claude_agent_sdk import UserMessage

        msg = MagicMock(spec=UserMessage)
        msg.parent_tool_use_id = "toolu_1"
        msg.content = []
    ...
    @pytest.mark.asyncio
    async def test_routes_system_message(self, connected_bridge, mock_conn):
    ...
    @pytest.mark.asyncio
    async def test_routes_result_message(self, connected_bridge, mock_conn):
    ...
    async def test_unknown_message_debug_log(self, mock_conn):
        ...
        msg = MagicMock()  # Not a known SDK type
...
class TestEmitAssistant:
    ...
    async def test_thinking_block_emits_agent_thought_chunk(
        ...
        thinking_block.thinking = "Let me analyze..."
    ...
    async def test_tool_use_block_caches_pending_tool(
        ...
        tool_block = MagicMock(spec=ToolUseBlock)
        tool_block.id = "toolu_cache_test"
    ...
    @pytest.mark.asyncio
    async def test_unknown_block_type_skipped(self, connected_bridge, mock_conn):
        """Unknown block types are skipped (not emitted via session_update)."""
        from claude_agent_sdk import AssistantMessage

        unknown_block = MagicMock()  # No matching spec
...
class TestEmitUserMessage:
    ...
    async def test_user_message_with_tool_id(self, connected_bridge, mock_conn):
        ...
        msg = MagicMock(spec=UserMessage)
        msg.parent_tool_use_id = "toolu_789"
        msg.content = []  # No error blocks
    ...
    async def test_user_message_correlates_pending_tool(
        ...
        msg = MagicMock(spec=UserMessage)
        msg.parent_tool_use_id = "toolu_corr"
        msg.content = []
    ...
    async def test_user_message_unknown_tool_id_title_none(
        ...
        msg = MagicMock(spec=UserMessage)
        msg.parent_tool_use_id = "toolu_unknown"
        msg.content = []
    ...
    async def test_user_message_error_result_sets_failed(
        ...
        msg = MagicMock(spec=UserMessage)
        msg.parent_tool_use_id = "toolu_err"
        msg.content = [error_block]
    ...
    async def test_user_message_success_result_sets_completed(
        ...
        ok_block = MagicMock(spec=ToolResultBlock)
        ok_block.is_error = False

        msg = MagicMock(spec=UserMessage)
        msg.parent_tool_use_id = "toolu_ok"
...
class TestInputJsonDelta:
    ...
    @pytest.mark.asyncio
    async def test_full_tool_use_streaming_flow(self, connected_bridge, mock_conn):
