...
# Research & Brainstorm Skill (spec-research)
...
**Announce at start:** "I'm using the `spec-research` skill to conduct structured research and brainstorming."

**Save findings to:** `<Research>` (`.vault/research/yyyy-mm-dd-<feature>-<phase>-research.md`)

**Dispatch sub-agent:** Invoke the `spec-subagent` skill with `adr-researcher`. Instruct it to "Conduct research on [topic]. Persist findings to .vault/research/..."
...
- Research & brainstorm might be followed by:
  - User approval -> proceed with `spec-adr` to create and persist `<ADR>`.
