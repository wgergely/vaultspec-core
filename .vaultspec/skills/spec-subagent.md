...
> **Warning:** This is a **utility skill** intended for use by other agents (e.g., `spec-execute`, `spec-research`). Users should generally **not** invoke this skill directly; instead, use the high-level workflow skills.
...
```bash
python .vaultspec/scripts/subagent.py run --agent adr-researcher --goal "Analyze the trade-offs of using 'Pattern A' vs 'Pattern B' for library crates."
```

**Dispatch a code review:**

```bash
python .vaultspec/scripts/subagent.py run --agent code-reviewer --goal "Review the `unsafe` block in `src/utils.rs`."
```
