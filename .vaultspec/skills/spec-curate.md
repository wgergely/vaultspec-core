...
# Documentation Curation Skill (spec-curate)
...
**Announce at start:** "I'm using the `spec-curate` skill to audit and clean the documentation vault."

## When to Use

- After completing a feature (post `spec-execute`) to verify documentation trail integrity.
...
Invoke the `spec-subagent` skill with `docs-curator`. Instruct it to "Perform a full vault audit of .vault/. Validate frontmatter, wiki-links, naming conventions, template compliance, and directory structure. Fix violations in-place and produce an audit report."
