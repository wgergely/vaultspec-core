...
**Announce at start:** "I'm using the `vaultspec-adr` skill to create and persist an `<ADR>`."
...
- **Derive from `<Research>`:** `<ADR>`s should always be preceded by a `vaultspec-research` session.
- **Dispatch Sub-Agent:** Invoke the `vaultspec-subagent` skill with `vaultspec-writer`. Instruct it to "Draft an ADR for [feature] based on the findings in [[...-research.md]]. Use the template at .vaultspec/templates/adr.md."
  - *Alternatives:* `vaultspec-adr-researcher` (if the research phase was skipped or needs synthesis).
