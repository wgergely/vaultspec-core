...
1. **System prompt delivery**: Neither provider uses the proper system prompt channel. Both prepend to user content.
1. **Streaming richness**: Claude emits `ToolCallProgress` with partial JSON args. Gemini emits `AgentPlanUpdate`, `CurrentModeUpdate`, etc. Neither covers the full ACP update spec.
1. **Session management**: Claude bridge implements 6 optional methods. Gemini added `load`/`resume` only in v0.28.0.
1. **Process architecture**: Claude is a 4-layer stack (orchestrator -> bridge -> SDK -> CLI -> API). Gemini is a 2-layer stack (orchestrator -> CLI -> API).
...
1. `test_readonly_write_blocked_both_providers` -- verify ACP write rejection
1. `test_streaming_events_both_providers` -- verify `AgentMessageChunk` from both
1. `test_cancel_both_providers` -- verify cancellation works
...
1. **`SubagentClient`** -- provider-agnostic, handles ALL ACP update types uniformly
1. **`run_subagent()`** -- truly provider-agnostic after `prepare_process()`
1. **`initial_prompt_override`** -- identical pattern for both providers
1. **`resolve_includes()`** -- shared utility in `base.py`, no duplication
1. **Client-side sandboxing** -- `write_text_file()` enforces read-only for BOTH via ACP
1. **Model registries** -- `ClaudeModels` / `GeminiModels` with `CapabilityLevel` mapping

### What Needs Work

1. **Terminal/shell escape from read-only** -- CRITICAL gap for both providers
1. **System prompt channels unused** -- both providers prepend to user content
1. **Zero cross-provider tests** -- parity regressions undetectable
1. **Undocumented env vars** -- `GEMINI_CWD` likely no-op
1. **Many SDK features unexposed** -- structured output, limits, hooks, sandbox
