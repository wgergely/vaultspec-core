...
1. The TypeScript SDK's `SDKMessage` type includes `session_id` on several message types
1. The first message that carries a `session_id` populates `session.claudeSessionId`
1. Subsequent `query()` calls pass this as `resume: session.claudeSessionId`
...
1. Store it in `_SessionState`
1. Pass `resume` to `ClaudeAgentOptions`
...
1. Fix cancel (per-session, await, don't kill client) -- unblocks reliable operation
1. Tool kind + rawInput + content -- improves ACP compliance for rich clients
1. Error stop reasons -- correctness
1. Content accumulation -- rich tool call display
1. TodoWrite -> Plan -- Zed plan panel support
1. Image/resource handling -- rich input support
1. Permission mode switching -- flexible operation
