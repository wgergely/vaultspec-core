...
1. **Session Recovery:** `load_session()` now gracefully recovers from bridge restarts by creating a new session state if the requested ID is unknown, rather than failing.
1. **Tool Error Visibility:** The bridge now explicitly handles `tool_use_error` events from the SDK/CLI and reports them as `failed` tool calls to the client, ensuring errors aren't swallowed.
