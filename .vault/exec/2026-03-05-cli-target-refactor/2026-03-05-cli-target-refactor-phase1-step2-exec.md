...
1. Cleaned up `args.root` and `args.content_dir` into `args.target_dir` in CLI entry points like `vault_cli.py` and `spec_cli.py`.
1. Updated tests in `tests/cli/test_vault_cli.py` and `tests/cli/test_spec_cli.py` to assert against `target_dir`.
1. Fixed up `tests/test_config.py` by removing validation and fallback testing for the deleted environment variables `VAULTSPEC_ROOT_DIR` and `VAULTSPEC_CONTENT_DIR`, successfully completing test execution.
