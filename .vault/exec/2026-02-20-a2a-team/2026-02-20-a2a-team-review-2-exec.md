...
1. **[Should Fix, Future]** Add error checking to `command_message` relay mode `resp.root.result` access (MEDIUM, carried forward).
1. **[Should Fix, Future]** Replace `asyncio.new_event_loop()` / `set_event_loop()` with `asyncio.run()` in CLI command handlers (MEDIUM, carried forward).
1. **[Optional, Future]** Add a public `TeamCoordinator.restore_session()` method to eliminate private attribute access from `scripts/team.py` (MEDIUM, carried forward).
1. **[Optional, Future]** Clear `self._clients` when HTTP client is recreated in `form_team()` API key override path (MEDIUM, carried forward).
