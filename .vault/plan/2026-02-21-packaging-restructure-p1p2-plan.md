...
2. **Move all library packages from `.vaultspec/lib/src/` to `src/vaultspec/`**
...
3. **Move CLI scripts as proper modules**
...
4. **Move test directories to top-level and adjust structure**
...
5. **Rewrite imports in leaf packages: `src/vaultspec/core/` and `src/vaultspec/vaultcore/`**
...
6. **Rewrite imports in mid-tier packages: `src/vaultspec/orchestration/`, `src/vaultspec/protocol/`, `src/vaultspec/hooks/`**
...
7. **Rewrite imports in analytics/RAG packages: `src/vaultspec/graph/`, `src/vaultspec/metrics/`, `src/vaultspec/verification/`, `src/vaultspec/rag/`**
   - Replace all bare-name imports with `vaultspec.*` prefixed forms
...
8. **Rewrite imports in server and CLI modules**
...
9. **Rewrite imports in all test files and conftest files**
...
10. **Update `pyproject.toml` for hatchling + uv**
...
11. **Add root package `__init__.py` and `__main__.py`**
...
12. **Fix `core/workspace.py` `_validate()` — remove `framework_root/lib/` check**
...
13. **Update `mcp.json` to use `uv run vaultspec-mcp`**
...
14. **Run `uv sync --dev` to verify editable install**
...
15. **Run `uv run pytest` — all tests must pass**
...
16. **Run `uv run python -m vaultspec --help` — CLI must work**
...
17. **Smoke test `uv run vaultspec-mcp`**
...
18. **Create `src/vaultspec/server.py` — unified FastMCP entry point**
...
19. **Refactor existing 5 tools from `subagent_server/server.py` into the unified server**
...
20. **Add stub tool router modules for future phases**
...
21. **Update all references from `vs-subagent-mcp` to `vaultspec-mcp`**
