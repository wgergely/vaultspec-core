...
2. Verify `/tmp/test-workspace/.vaultspec/rules/system/framework.md` and `/tmp/test-workspace/.gemini/rules/` exist.
3. Run `uv run vaultspec doctor --target /tmp/test-workspace` and verify the new "Workspace Root: /tmp/test-workspace" diagnostic appears.
4. Run `uv run pytest` and confirm all tests pass using the new `CliRunner` architecture.
5. Run `uv run vaultspec mcp --target /tmp/test-workspace` and verify zero logs leak to `stdout`. 
