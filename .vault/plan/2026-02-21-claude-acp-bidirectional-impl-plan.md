...
2. **Fix exception masking -- differentiate error types**
...
3. **Fix non-standard stop reasons on `ResultMessage` errors**
...
4. **Clear stale dicts between turns**
...
5. **Fix CLAUDECODE env var mutation in A2A executor**
...
2. **Store SDK client in per-session state**
...
3. **Resolve SDK client from session state in `prompt()`**
...
4. **Update `load_session()` and `resume_session()` to store per-session client**
...
5. **Update `fork_session()` to use per-session client**
...
6. **Update `SDKClientRecorder` in test conftest**
...
2. **Pass `resume` parameter in `load_session()` and `resume_session()`**
...
3. **Handle stale/expired sessions gracefully**
...
2. **Fix `cancel()` to use per-session client and async `interrupt()`**
...
3. **Remove `disconnect()` from cancel flow**
...
2. **Add `_get_tool_call_content()` for Edit/MultiEdit diff blocks**
...
3. **Emit enriched `ToolCallStart` in `_emit_assistant()`**
...
4. **Emit enriched `ToolCallProgress` in `_emit_user_message()`**
...
2. **Intercept TodoWrite in `_emit_stream_event()`**
...
3. **Suppress TodoWrite from `_emit_user_message()`**
...
4. **Import `AgentPlanUpdate` in bridge module**
...
2. **Multi-turn session resume tests**
...
3. **Per-session state isolation tests**
...
4. **Cancel/abort non-destructive tests**
...
7. **TodoWrite plan conversion tests**
