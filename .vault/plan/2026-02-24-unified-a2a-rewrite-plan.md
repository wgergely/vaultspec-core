...
`[[2026-02-24-subagent-protocol-adr]]`,
`[[2026-02-24-subagent-protocol-server-architecture-adr]]`, and
`[[2026-02-24-subagent-protocol-server-manager-adr]]`.
...
- Phase 0 — Pre-flight & Cleanup
    ...
    2. Run full existing test suite, capture baseline
    3. Verify `a2a-sdk` is at latest version in `pyproject.toml`
...
- Phase 2 — A2A Server Entry Point
    ...
    2. Add `protocol/a2a/__main__.py`
    3. Manual: start server, verify agent card at
       `/.well-known/agent-card.json`

- Phase 3 — Rewire Providers
    1. `providers/gemini.py` — update `prepare_process()` to A2A server args
    2. `providers/claude.py` — update `prepare_process()` to A2A server args
    3. `providers/base.py` — add `protocol` field to `ProcessSpec`
    4. Update `protocol/tests/test_providers.py`
...
- Phase 4 — Rewire `run_subagent()`
    ...
    2. Move `SubagentResult`, `SubagentError` out of `acp/` to
       `protocol/types.py` (keep backward-compat re-exports temporarily)
    3. Rewrite `run_subagent()` — remove all ACP imports, use
       `ServerProcessManager` + `A2AClient`
    4. Update `TeamCoordinator` to use `ServerProcessManager`
    5. Update `orchestration/__init__.py` exports
    6. Write tests — mock `ServerProcessManager`, mock `A2AClient` via
       `respx`, test happy path / timeout / cancel / error

- Phase 5 — Delete ACP Layer
    1. Verify no production imports from `protocol.acp`
    2. Delete `protocol/acp/` entirely
    3. Update `protocol/__init__.py` — remove `from .acp import` lines
    4. Remove ACP-specific test utilities
    5. Clean up `pyproject.toml` if needed

- Phase 6 — Validation & Polish
    1. Run full test suite
    2. Manual: spawn real Gemini A2A subagent
    3. Manual: A2A Inspector test
    4. Manual: run subagent via CLI
    5. Verify team coordination still works
    6. Write walkthrough
