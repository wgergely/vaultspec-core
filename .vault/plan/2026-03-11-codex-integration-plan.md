...
- Phase 2: Decouple destination-path assumptions from a single provider root
  ...
  1. Rework the relevant sync and config-generation paths in
  ...
  1. Decide and encode how root `AGENTS.md` sync, markdown config generation,
     and system-prompt generation coexist without overloading `Tool.AGENTS`
     and `Resource.AGENTS`.

  1. Define the minimum resource-model changes needed for Antigravity
...
- Phase 3: Add Codex-native support
  ...
  1. Implement Codex instruction support around the existing `AGENTS.md`
     bridge and ensure it composes cleanly with current AGENTS sync behavior.

  1. Reuse skill sync where possible, but retarget Codex skills to
     `.agents/skills/<skill>/SKILL.md` and verify the resulting layout matches
     Codex expectations.

  1. Scope Codex TOML integration explicitly: either implement a safe
     `.codex/config.toml` mutation path now or leave it out of the first
     rollout behind a clear boundary in code and tests.

  1. Confirm that Codex does not participate in markdown config generation,
...
- Phase 4: Verification and regression coverage
  ...
  1. Add targeted tests for path/config assembly in
  ...
  1. Add scaffolding coverage for `init_run()` to verify `.agents/*`
  ...
  1. Add end-to-end dry-run or temp-workspace sync checks that validate the
     actual on-disk outputs for Claude, Gemini, Antigravity, and Codex side
     by side.

  1. Update any user-facing documentation touched by the rollout so provider
