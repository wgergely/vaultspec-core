...
1. `rm -rf tests/subagent` (Empty after move).
1. `rm -rf tests/integration` (Empty after move).
1. `rm -rf tests/cli` (Empty after move).
1. `rm -rf tests/rag` (Empty after move).
...
1. `mkdir -p tests/protocol/cross`
1. **Fixtures**: Create `tests/protocol/conftest.py`.
...
1. **Claude Subagent**: `tests/protocol/isolation/test_subagent_claude.py`
   - Same as Gemini.
1. **Gemini Team**: `tests/protocol/isolation/test_team_gemini.py`
   - `test_team_lifecycle`: Form team (2x Echo Gemini) -> Dispatch -> Dissolve.
1. **Claude Team**: `tests/protocol/isolation/test_team_claude.py`
   - Same as Gemini.
...
1. **Mixed Team**: `tests/protocol/cross/test_team_mixed.py`
...
1. Run `pytest tests/protocol/` to verify the new matrix. (Expect some failures initially if providers have bugs, but structure is verified).
