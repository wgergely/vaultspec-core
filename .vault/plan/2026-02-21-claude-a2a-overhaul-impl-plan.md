...
- `Phase 1: SDK Fix + Executor Hardening (ADR Decisions 1 + 2)`
  ...
  1. **Switch to `receive_response()`**: Replace the manual
  ...
  1. **Add bounded retry**: Add `max_retries: int = 3` and
  ...
  1. **Add session resume**: Add `_session_ids: dict[str, str]` to `__init__()`.
  ...
  1. **Make cancel non-destructive**: Remove `disconnect()` from `cancel()`. Keep
  ...
  1. **Add streaming progress**: Emit `updater.update_status()` with accumulated
  ...
  1. **Extend `_InProcessSDKClient`**: Add `receive_response()` method. Add ability
     to inject errors (for retry testing) and `session_id` on `ResultMessage`
     (for resume testing).

  1. **Write tests**: Retry after rate limit. Failure after max retries. Session
...
- `Phase 2: Implement team_tools.py (ADR Decision 3)`
  ...
  1. **Implement `create_team()`**: Accept `name: str` and `agent_urls: list[str]`.
  ...
  1. **Implement `team_status()`**: Accept `name: str`. Load session from
     `.vault/logs/teams/{name}.json`. Return status, members, and their states.

  1. **Implement `list_teams()`**: List `.json` files in `.vault/logs/teams/`.
     Return list of team names and statuses.

  1. **Implement `dispatch_task()`**: Accept `team_name: str`, `agent_name: str`,
     `task: str`. Restore coordinator from session. Call
     `dispatch_parallel({agent_name: task})`. Return task ID and status.

  1. **Implement `broadcast_message()`**: Accept `team_name: str`, `message: str`.
     Dispatch same message to all members. Return results.

  1. **Implement `send_message()`**: Accept `team_name: str`, `to: str`,
     `message: str`. Dispatch to specific agent. Return result.

  1. **Implement `dissolve_team()`**: Accept `team_name: str`. Restore coordinator.
     Call `dissolve_team()`. Delete session file. Return confirmation.

  1. **Write tests**: Test each tool function with DI'd coordinator. Test session
...
- `Phase 4: Wiring (ADR Decisions 5 + 6)`
  ...
  1. **Document MCP injection pattern**: Add a helper or documentation showing how
  ...
  1. **Verify MCP tool discovery**: Start the unified server, connect a client,
...
- `Phase 5: Integration Testing`
  ...
  1. **Team tools integration test**: Verify `create_team` → `dispatch_task` →
     `team_status` → `dissolve_team` flow through MCP tools.

  1. **Executor + team tools test**: Construct executor with `mcp_servers` including
     team tools. Verify the executor can be configured to access team capabilities.

  1. **Run full test suite**: All non-E2E tests pass. No regressions.
