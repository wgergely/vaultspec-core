...
- **Wave 1** -- Phase 1: Leaf packages `core/`, `vaultcore/`, `hooks/` (1 agent, sequential)
  ...
  1. **Step 2**: Rewrite all consumers of `core/`, `vaultcore/`, `hooks/` to use package-level imports
...
- **Wave 2** -- Phases 2+3: Mid-tier analytics + RAG (2 agents in parallel)
  ...
  1. **Step 3** (Agent A): Add `__all__`, `__init__.py` re-exports for `graph/`, `metrics/`, `verification/` and rewrite their consumers
  ...
  1. **Step 4** (Agent B): Add `__all__`, lazy `__init__.py` for `rag/` and rewrite consumers
...
- **Wave 3** -- Phase 4: `orchestration/` (1 agent)
  ...
  1. **Step 5**: Add `__all__`, selective `__init__.py` for `orchestration/` and rewrite consumers
...
- **Wave 4** -- Phase 5: `protocol/` (1 agent)
  ...
  1. **Step 6**: Add `__all__`, `__init__.py` re-exports for entire `protocol/` hierarchy and rewrite consumers
...
- **Wave 5** -- Phase 6: `subagent_server/` (1 agent)
  ...
  1. **Step 7**: Add `__all__`, `__init__.py` re-exports for `subagent_server/` and rewrite consumers
...
- **Wave 6** -- Phase 7: Entry points + tests (1 agent, sequential)
  ...
  1. **Step 8**: Retarget entry point imports and add `__all__` to top-level modules
  ...
  1. **Step 9**: Rewrite all test imports to use package-level imports
...
- **Wave 7** -- Verification (1 agent)
  ...
  1. **Step 10**: Full verification pass
