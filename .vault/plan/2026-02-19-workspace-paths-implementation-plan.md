...
- `Phase 1: Core Module + Config` — Foundation with no external impact
    ...
    2. **Add `content_dir` to `core/config.py`** — Add `content_dir: Path | None = None` field to `VaultSpecConfig` dataclass (after `root_dir`). Add corresponding `ConfigVariable` entry to `CONFIG_REGISTRY` with `env_name="VAULTSPEC_CONTENT_DIR"`, `var_type=Path`, `default=None`.
...
- `Phase 3: Packaging + Extension Manifest` — Non-code deliverables
    ...
    2. **Create `extension.toml`** — New file at repo root. Follow the schema from ADR-006: `[extension]` (name, version, description), `[requires.python]` (version), `[runtime]` (type, install), `[entry_points]` (cli, mcp), `[provides]` (mcp, content_types), `[outputs]` (claude_dir, gemini_dir, agent_dir, agents_md).
