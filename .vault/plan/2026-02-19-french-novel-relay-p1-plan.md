...
- Phase 1: File scaffolding and shared infrastructure
  ...
  1. Define `_send_message_payload(text)` and `_build_client(executor, name, port)`
     helpers — identical to `test_e2e_a2a.py` (copy the pattern, these are
     module-local helpers not shared via conftest).

  1. Define `_extract_response_text(body)` helper — extracts text from the
     JSON-RPC response body (`body["result"]["status"]["message"]["parts"][0]["text"]`).
     Returns empty string on any missing key.

  1. Define `FRENCH_WORDS` frozenset containing French indicator words (`le`,
     `la`, `les`, `un`, `une`, `de`, `du`, `des`, `et`, `est`, `dans`, `sur`,
     `qui`, `que`, `pas`, `avec`, `pour`, `mais`).

  1. Define `_assert_french_prose(text, min_len=100, max_len=2000)` validation
  ...
  1. Define three French prompt constants: `PROMPT_BEGIN` (Turn 1 — Claude
...
- Phase 2: StoryRelayExecutor and mock test class
  ...
  1. Define three chapter template constants: `MOCK_CHAPTER_1` (French
  ...
  1. Implement `TestFrenchNovelRelayMock` class with `@pytest.mark.integration`
  ...
  1. Assertions for mock test: all 3 responses have `state == "completed"`,
...
- Phase 3: Live test class
  ...
  1. Single async test method `test_three_turn_french_story`. Set up Claude
  ...
  1. Turn 1: Send `PROMPT_BEGIN` to Claude client. Extract `chapter_1` via
     `_extract_response_text()`. Assert state completed. Call
     `_assert_french_prose(chapter_1)`. Print timing.

  1. Turn 2: Format `PROMPT_CONTINUE` with `chapter_1`. Send to Gemini client.
  ...
  1. Turn 3: Format `PROMPT_FINISH` with `chapter_1` and `chapter_2`. Send to
