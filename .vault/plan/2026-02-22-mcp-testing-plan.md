...
- Task 1: Create fixtures
    ...
    2. `client_session_no_lifespan` fixture — bare `FastMCP` with tools
       registered but no lifespan, for focused tests
    3. Helper fixtures: `baker_cache`, `fresh_engine` (reuse existing
...
- Task 2: Protocol handshake + discovery tests
    ...
    2. `TestProtocolHandshake::test_capabilities_include_tools` — tools cap
    3. `TestProtocolHandshake::test_capabilities_include_resources` — resources cap
    4. `TestToolDiscovery::test_all_15_tools_listed` — count + names
    5. `TestToolDiscovery::test_tool_schemas_present` — inputSchema on each
    6. `TestToolDiscovery::test_tool_annotations_present` — annotations set
    7. `TestResourceDiscovery::test_agent_resources_listed` — agents:// URIs
    8. `TestResourceDiscovery::test_read_agent_resource` — returns JSON

- Task 3: Subagent tool round-trip tests
    1. `TestSubagentToolsRoundTrip::test_list_agents` — call through protocol
    2. `TestSubagentToolsRoundTrip::test_dispatch_agent` — dispatch + taskId
    3. `TestSubagentToolsRoundTrip::test_get_task_status` — poll status
    4. `TestSubagentToolsRoundTrip::test_cancel_task` — cancel a task
    5. `TestSubagentToolsRoundTrip::test_get_locks` — empty locks list

- Task 4: Team tool round-trip tests
    1. `TestTeamToolsRoundTrip::test_list_teams` — empty list
    2. `TestTeamToolsRoundTrip::test_list_teams_after_create` — if feasible

- Task 5: Error propagation tests
    1. `TestErrorPropagation::test_unknown_tool_is_error`
    2. `TestErrorPropagation::test_dispatch_unknown_agent_is_error`
    3. `TestErrorPropagation::test_get_status_missing_task_is_error`
    4. `TestErrorPropagation::test_cancel_missing_task_is_error`
