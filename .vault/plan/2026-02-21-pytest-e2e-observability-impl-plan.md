...
- `Phase 1 — Infrastructure`
  ...
  1. `Add new test dependencies` — Add `pytest-rerunfailures>=16.0`,
  ...
  1. `Install new dependencies` — Run `uv sync --group dev` to install
     the new packages.
...
- `Phase 2 — Test Instrumentation`
  ...
  1. `Add results_bag metrics to E2E tests` — Instrument the test methods
  ...
  1. `Add logging to E2E test files` — Add `import logging` and
...
- `Phase 3 — Housekeeping`
  ...
  1. `Verify fast tests still pass` — Run
