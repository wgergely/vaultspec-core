...
1. Adding Claude behavioral rule generation to the sync pipeline
1. Supporting explicit `order:` frontmatter for assembly control
1. Relocating verbose shell examples from system prompt to skill files
1. Fixing all small defects (typos, allcaps, forward-references)
...
- Phase 1: Content restructuring (system/ files)
  ...
  1. **Clean `base.md`**: Remove "Jean-Claude" persona name from line 1.
  ...
  1. **Add `order:` frontmatter** to `workflow.md`: `order: 20`. This ensures
     workflow routing appears early in assembly (after base + tool-specific).

  1. **Fix defects in `operations.md`**:
  ...
  1. **Fix forward-reference in `gemini.md`**: Line 64, change "per the
...
- Phase 2: CLI pipeline changes (`cli.py`)
  ...
  1. **Add `_generate_system_rules`** function. New function that assembles
  ...
  1. **Update `system_sync`** to also generate `.claude/rules/vaultspec-system.builtin.md`
  ...
  1. **Update `sync_all`**: No changes needed -- `system_sync` already runs
...
- Phase 3: Shell example relocation
  ...
  1. **Enrich skill files**: Verify each skill file (`vaultspec-fd.md`,
...
- Phase 4: Test updates
  ...
  1. **Update `test_sync_operations.py`** `TestSystemSync`:
  ...
  1. **Add `test_pipeline_config_excluded_from_rules`**: Verify files with
...
1. Zero Gemini-specific tool names (`run_shell_command`, `save_memory`,
   `read_file`, `activate_skill`, `/help`, `/bug`) in any shared system/ file
   (files without `tool:` frontmatter).

1. All 87+ existing CLI tests pass.

1. New tests cover: order frontmatter, Claude rule generation, tool-specific
   exclusion from rules, pipeline:config exclusion from rules.

1. `gemini.md` shell section is \<=15 lines (down from ~45).

1. No `****` typo, no allcaps warnings, no forward-references in assembled
   -utput.

1. Claude CLAUDE.md + rules contain: vaultspec pipeline bootstrap (frontmatter)

   - skill/subagent rules + behavioral mandates (new rule file).

1. `markdownlint` passes on all modified files.
