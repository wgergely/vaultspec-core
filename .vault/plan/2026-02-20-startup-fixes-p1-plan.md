...
- `Fix 1: mcp import profiling and lazy registration`
    ...
    2. Investigate FastMCP's internal tool-registration API: determine whether
       `@mcp.tool` schema generation can be deferred until first tool invocation.
    3. If deferral is feasible: refactor tool declarations in `server.py` to use
       lazy registration; confirm schemas are still generated correctly.
    4. If deferral is not feasible: profile whether moving `from mcp.server.fastmcp
       import FastMCP` to a background thread (while the process accepts the stdio
       pipe) reduces perceived startup latency.
    5. Re-benchmark and record the new cold-start time.

- `Fix 2: ACP handshake resilience`
    1. Locate the `conn.initialize()` call in `run_subagent()` (subagent.py:293).
    2. Wrap it with `asyncio.wait_for(conn.initialize(...), timeout=cfg.acp_handshake_timeout)`
       where `acp_handshake_timeout` is a new config key (default: 30s, configurable).
    3. On `asyncio.TimeoutError`: raise `SubagentError` with message
       `"ACP handshake timed out after {N}s — is the claude/gemini binary installed?"`.
    4. Log the timeout event via `logger_instance` as `handshake_timeout` with
       `elapsed` and `timeout` fields (matches the existing JSONL schema observed
       in `.vault/logs/`).
    5. Add pre-warm in `_server_lifespan()`: on MCP server startup, spawn the
    ...
    6. Add a `acp_handshake_timeout` key to `core/config.py` with default 30.0.
...
- `Fix 3: torch initialization guard`
    ...
    3. If probe succeeds, proceed with the regular `from rag.api import index`
       import (which will now be fast, since torch is warm in the OS).
    4. Alternatively: move the torch import into a `concurrent.futures.ThreadPoolExecutor`
       call with `timeout=60` to allow the main thread to remain responsive.
    5. Add `rag_timeout` config key (default: 60s) to `core/config.py`.
    6. Update `rag/embeddings.py` to call `torch.cuda.init()` explicitly and
       catch any `RuntimeError` to fall back to CPU mode gracefully.
    7. Write a test in `lib/tests/` or `rag/tests/` that verifies the timeout
...
- `Fix 4: documentation update`
    ...
    2. Add a "RAG / GPU Requirements" callout: Python 3.13+, CUDA-capable GPU,
       matching PyTorch CUDA wheel.
    3. Add MCP server startup section to README: clarify the MCP server has no
       PyTorch dependency and starts in ~1s.
    4. Review `pyproject.toml` optional-deps comments for clarity.
...
vaultspec` produces a working MCP server with no `torch`/`lancedb` installed.
