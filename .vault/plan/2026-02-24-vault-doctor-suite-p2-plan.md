...
- `vaultspec vault doctor --category links` against a vault containing a doc
  that links to a non-existent target emits `Severity.ERROR` with `check =
  "broken-wikilinks"`.
...
- `vaultspec vault doctor --category links` against a vault with a `related`
  entry of `"not-a-wikilink"` emits `Severity.ERROR` with `check =
  "malformed-related"` and `fix_available = True`.
