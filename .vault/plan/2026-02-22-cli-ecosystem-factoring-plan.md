...
1. Rename directory and move tests
...
1. Update all 36 consuming files
...
1. Place deprecation shim at `src/vaultspec/core/__init__.py`
...
1. Refactor `cli.py` to use `cli_common`
...
1. Refactor `subagent_cli.py` to use `cli_common`
...
1. Refactor `team_cli.py` to use `cli_common`
...
1. Refactor `vault_cli.py` to use `cli_common`
...
1. Create `core/helpers.py` -- depends on types
...
1. Create `core/sync.py` -- depends on types, helpers
...
1. Create `core/rules.py`, `core/agents.py`, `core/skills.py`
...
1. Create `core/config_gen.py` and `core/system.py`
...
1. Create `core/__init__.py` with public API re-exports
...
1. Slim down `cli.py` to thin wrapper
...
1. Update `src/vaultspec/tests/cli/conftest.py` and test imports
...
1. Delete silent `PROVIDERS = {}` fallback and fix logger NameError
...
1. Move `import html` to top-level stdlib imports
...
1. Delete `sys.exit(1)` guards in `subagent_cli.py` and `team_cli.py`
