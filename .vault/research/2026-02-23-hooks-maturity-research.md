...
2. **Agent dispatch fix** — use `vaultspec subagent run` CLI, fix `--task`→`--goal`
3. **Process safety** — kill timed-out subprocesses, prevent zombies
4. **Platform safety** — fix Windows path handling in shell actions
5. **Deduplication** — prevent `.yaml`/`.yml` double-loading
6. **Remove dead code** — drop `vault.document.modified` or implement it, remove YAML fallback
7. **Fix tests** — add agent action tests, fix `test_failing_command`, add integration tests
8. **Documentation** — hooks guide, README mention, CLI reference expansion, example improvements
