...
1. **Structure:** `skills-ref` wraps the list in `<available_skills>`.
1. **Fields:** `skills-ref` includes a `<location>` field pointing to the `SKILL.md` file, which is critical for the "progressive disclosure" pattern (the agent knows *where* to read the full instructions). `vaultspec` currently omits the location.
1. **Parsing:** XML is generally more robustly parsed by Claude models for tool/skill definitions than unstructured Markdown lists.
...
1. **Include Location:** Resolve the absolute or relative path to `SKILL.md` for each skill.
1. **Use `skills-ref` logic:** Mimic the logic from `skills_ref.prompt.to_prompt`.
...
1. Plan the refactor of `src/vaultspec/cli.py`.
