...
1. Codex does **not** document a `CODEX.md` or a `.codex/rules/` directory.
...
1. Codex skills are documented under **`.agents/skills/`**, not under
...
1. Codex supports agent definitions, but the shape is **TOML config tables**
   inside `config.toml`, not a synced `agents_dir` of markdown files.

1. Codex frontmatter use is narrow:
...
1. Decide whether vaultspec will support non-markdown config mutation for
   `.codex/config.toml`.

1. Correct Antigravity's local model from `.agent` to the officially documented
   `.agents` workspace layout unless contradictory primary-source evidence is
   found.

1. Decide whether Antigravity needs a dedicated `workflows` resource in
   vaultspec, since the current resource model has no equivalent.

1. Revalidate whether Antigravity rule files require or tolerate vaultspec's
