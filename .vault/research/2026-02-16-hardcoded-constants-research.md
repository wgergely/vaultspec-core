...
## Recommended Environment Variables (VAULTSPEC\_ Prefix Convention)
...
1. Add `VAULTSPEC_MCP_HOST` env var (currently hardcoded to 0.0.0.0)
1. Add `VAULTSPEC_EMBEDDING_BATCH_SIZE` env var (already parameterized)
1. Verify `VS_MCP_TTL_SECONDS` documentation

### Phase 2: Medium-Risk Refactors

1. Create `vaultspec/config.py` with centralized constants module
1. Refactor `.lance` path to use `VAULTSPEC_LANCE_DIR` env var
1. Refactor `.vault` path to use `VAULTSPEC_DOCS_DIR` env var
1. Add host/port configuration for A2A modules

### Phase 3: Framework Restructuring

1. Support `VAULTSPEC_FRAMEWORK_DIR` for relocatable framework
1. Support tool-specific directory env vars (`.claude`, `.gemini`, `.agent`)
1. Update CLI to use environment variables as fallback for all paths

### Phase 4: Documentation & Testing

1. Create `.env.example` with all Tier 1-2 env vars
1. Add env var reference to README
1. Add integration tests for env var fallback paths
...
1. **Implementer**: Start with Phase 1 (MCP port/host) for quick wins
1. **QA**: Validate env var fallback paths don't break existing workflows
1. **DevOps**: Prepare deployment configuration with standardized env vars
