...
1. **"Running vaultspec with Gemini CLI"** — ACP integration walkthrough; ACP is a
   key differentiator and the tutorial corpus should demonstrate it

1. **"Multi-Agent Workflow: Research Agent Calls Executor"** — shows the MCP subagent
   server in action, the most technically impressive feature

1. **"Customizing Agents: Creating Your Own Executor"** — how to define a new agent with
...
1. Add a "Before / After" section to the README: show what AI development looks like
   without vaultspec (context loss, inconsistent decisions, no audit trail) vs. with it.

1. Ensure the README's documentation links section surfaces `docs/tutorials/` and
   `docs/guides/` — not just the top-level docs.

**Tier 2: Tutorial expansion (needed for developer adoption)**

1. Write "Adding vaultspec to an Existing Project" tutorial.
1. Write "Running vaultspec with Gemini CLI" tutorial (ACP differentiator showcase).

**Tier 3: Internal hygiene (needed for credibility)**

1. Strip Rust-specific language from `vaultspec-adr-researcher.md` and
   `vaultspec-complex-executor.md` (Wave 1.4).

1. Add a "See Also: Human Documentation" section to `.vaultspec/README.md` linking to
   `docs/getting-started.md`, `docs/concepts.md`, and `docs/tutorials/`.

1. Document or scope the four utility skills (`fd`, `rg`, `sg`, `sd`) in `cli-reference.md`.

**Tier 4: Navigation polish (nice to have)**

1. Add a `docs/README.md` index listing all documentation with one-line summaries and
