...
1. **Security:** Injection risks, secrets, permissions.
1. **Robustness:** Error handling, validation, resource management.
...
1. **Phase 2: Vault Operations** (`vaultcore`, `graph`, `verification`, `vault_cli.py`)
1. **Phase 3: Protocol & Orchestration** (`protocol`, `orchestration`, `subagent_server`)
1. **Phase 4: RAG & Metrics** (`rag`, `metrics`)
1. **Phase 5: Tools & Hooks** (`mcp_tools`, `hooks`)
