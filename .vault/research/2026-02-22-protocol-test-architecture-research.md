...
1. **Observability**: Debugging multi-agent failures requires deep tracing of the message exchange, not just the final output.
...
1. **Simple Payloads**: Use "Echo", "Hello World", or "State Retention" tasks. No creative writing.
1. **Strict Separation**: Isolation tests vs. Cross-wiring tests.
...
1. **Delete** the legacy `tests/` directories.
1. **Implement** the new `tests/protocol/` suite from scratch.
