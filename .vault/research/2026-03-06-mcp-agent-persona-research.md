...
1. Replace placeholders: `{feature}`, `{yyyy-mm-dd}`, `{topic}`, etc.
1. Validate filename using `VaultConstants.validate_filename`.
1. Write atomically using `vaultspec.core.helpers.atomic_write`.
...
1. Parse metadata.
1. Filter by `type` (directory tag) and `feature` (feature tag).
1. Perform case-insensitive string containment check on title/body for `query`.
...
1. **`get_feature_status(feature: str)`**

   - Returns: `{ feature, status, documents: { research: [], adr: [], ... } }`.

1. **`create_document(type: str, feature: str, title: str, extra_context: str)`**
