...
2. Do not introduce a fake `CODEX.md` or `.codex/rules/` abstraction.
3. Treat `.agents/skills/` as the correct future target if Codex skills are
   later synchronized.
4. Treat `.codex/config.toml` mutation as a separate explicit capability,
   rather than assuming it falls out of the current `_generate_config()` path.
5. Normalize Antigravity to the officially documented `.agents` workspace
   layout and `GEMINI.md` instruction model before adding Codex as a first-
   class destination.
6. Defer decisions about Antigravity workflows and Codex TOML mutation to the
