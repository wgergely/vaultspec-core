...
1. **Session Robustness:** The reference creates a new session if `loadSession` is called for an unknown ID (handling bridge restarts). The bridge fails.
1. **Error Handling:** The reference handles `tool_use_error` stream events. The bridge might miss these.
...
1. **Session Recovery in `load_session`:**
