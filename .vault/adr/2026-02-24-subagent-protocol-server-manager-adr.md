...
# `subagent-protocol` adr: Shared `ServerProcessManager` | (**status:** `accepted`)

## Problem Statement

With A2A requiring per-agent server processes (see
`[[2026-02-24-subagent-protocol-server-architecture-adr]]`), something must
manage the lifecycle of these processes: spawning, readiness probing, port
discovery, shutdown, and orphan prevention. Two code paths currently need
this capability:
