...
2. `.gitignore` excludes generated content:
...
3. No changes to `_builtins_root()`. The existing dual-path logic is correct:
...
4. No changes to runtime sync. All sync operations read from `TARGET_DIR/.vaultspec/rules/<category>/`, never from the installed package. The builtins module is only consulted during `install` and `install --upgrade`.
