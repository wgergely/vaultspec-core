...
1. Constructs a `UserMessage` with a mix of text and image blocks.
1. Converts resource URIs (e.g., `file:///foo`) into text pointers (e.g., `@/foo`) which the Claude CLI understands.
...
1. **Refactor Prompt Construction**: Replace `_extract_prompt_text` with `_build_sdk_message_content(prompt)`. This function will iterate over the ACP content blocks and produce a list of blocks suitable for the Claude SDK:
