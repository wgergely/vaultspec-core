...
2. Neither `GEMINI_API_KEY` nor `~/.gemini/oauth_creds.json` exists -- the binary falls into an interactive browser OAuth flow, hanging the headless ACP session silently.
3. `GEMINI_API_KEY` is set but invalid -- the binary fails with no vaultspec-level guidance on corrective action.
...
2. If neither is present, reads `~/.claude/.credentials.json`, extracts the `accessToken` (short-lived, ~1h), and injects it as `CLAUDE_CODE_OAUTH_TOKEN`.
3. Logs a warning if the file is absent or the token is missing.
...
2. POST to `token_uri` (typically `https://oauth2.googleapis.com/token`) with:
   - `grant_type=refresh_token`
   - `refresh_token=<value>`
   - `client_id=<value>`
   - `client_secret=<value>`
3. On success: update `access_token` and `expiry` in the JSON, write back to the file atomically (write to a temp file alongside, then `os.replace()`).
4. On failure: log `logger.warning("Gemini OAuth token refresh failed: %s. Subprocess may fail to authenticate.")` and continue.
...
2. Token expiry is the primary failure mode. Proactive refresh before spawn eliminates the silent-hang failure class without requiring user intervention on a per-session basis.
