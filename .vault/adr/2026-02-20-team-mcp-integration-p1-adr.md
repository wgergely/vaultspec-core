...
2. Add `serve` subcommand to `team.py` (or add a new `agents.py` script) that calls
...
4. Create `.vaultspec/rules/skills/vaultspec-team.md` using the structural template
   derived from `vaultspec-subagent.md` (72-line pattern, 5 sections, same tooling block).

5. Update all 13 rules files containing `vaultspec-subagent` mentions (22 locations) with
   conditional language distinguishing single-agent from team dispatch.

6. Update `vaultspec-subagents.builtin.md` (both copies) to reference `vs-agents-mcp` and
