...
1. `CLAUDE_CODE_OAUTH_TOKEN` (subscription billing, purpose-built for headless use)
1. `~/.claude/.credentials.json` (OAuth credentials file, requires interactive refresh)
...
1. Check `expiresAt` (ms): if `expiresAt / 1000 > time.time() + buffer`, the
   `accessToken` is still valid — inject it as `CLAUDE_CODE_OAUTH_TOKEN`.

1. If expired, use `refreshToken` to POST to Anthropic's OAuth token refresh endpoint.
