...
1. Core framework rules still claim a team-thread MCP orchestration surface that the shipped product does not expose.
...
1. `.env.example` still documents an older runtime model and stale product name.
...
1. Generated/internal framework surfaces still use `vaultspec` as the framework identity in ways that blur product name versus framework name.
...
1. CLI reference still contains known-drift notes that must be reconciled with code fixes, not left as permanent wrapper text.
   Type: documentation defect
...
1. Some prose still says "vaultspec workspace" or "vaultspec framework."
   Type: editorial only
...
1. Rewrite the framework/rule files that still promise MCP team-thread orchestration.
   Reason:

   - these create semantic drift in the project brain, not just stale docs

1. Re-audit generated/internal framework outputs after the source-rule repair.
   Reason:

   - `.agents/` and `.claude/` should inherit the corrected semantics

1. Do a final wrapper-doc pass over README and CLI/MCP docs only after the
