...
2. A user attempting MCP setup can follow obsolete instructions and fail before
   transport initialization.
3. A user reading the current docs can reasonably expect commands and
   capabilities that do not exist.
4. Repo boundary ambiguity makes it unclear whether `vaultspec-core` is a
...
2. What it installs.
   ...
   `vaultspec-mcp`.
3. Requirements and recommended install path.
   Lead with the recommended standalone install path, `pipx install
   vaultspec-core`. Include the Python version requirement. Include source
...
1. There is no clear user-facing install section in the current landing
   documentation.
2. The documentation does not recommend a default install path for end users.
   The preferred default should be `pipx install vaultspec-core`.
3. There is no explicit quickstart explaining the workspace precondition and the
   role of `vaultspec init`.
4. The current user-facing surface does not explain that `vaultspec-mcp` is a
   separate installed executable.
5. MCP setup guidance does not currently document the required environment,
   especially `VAULTSPEC_TARGET_DIR`.
6. Package metadata points the Documentation URL at a missing top-level `docs/`
   directory even though the live documentation lives under `.vaultspec/docs/`.
7. Build guidance, if included anywhere user-facing, should use `python -m
   build` rather than legacy packaging flows.
...
2. The executable is `vaultspec-mcp`.
3. Client configuration must document `command`, `args`, required `env`, and
   optional `cwd`.
4. Paths should be absolute where path resolution matters.
5. The minimum required environment should include `VAULTSPEC_TARGET_DIR` when
   the client process is not already rooted in the intended workspace.
6. The docs should briefly explain that the client launches the server as a
   subprocess and exchanges JSON-RPC messages over stdin/stdout.
7. The docs should warn that non-protocol stdout breaks the transport and that
   logs belong on stderr.
8. The root README should include one minimal client example only, with deeper
   client-specific guidance moved to linked documentation.
9. Verification guidance should include a concrete path such as MCP Inspector.
10. The documented tool list should match the live vault-oriented MCP surface
...
2. `cli.py` contains dead special-case logic for `mcp`.
3. `doctor_run()` crashes on Windows `cp1252` consoles because of Unicode
   checkmark output.
4. The hooks engine docstring path differs from the runtime path wiring.
...
1. Fix the root `README.md` first so the package landing page states the correct
   ...
   map.
2. Rewrite `.vaultspec/docs/cli-reference.md` against the live CLI and MCP
   surfaces. This is the highest-drift reference document and should become the
   source of truth for command behavior.
3. Rewrite `.vaultspec/docs/concepts.md` so the mental model and walkthrough use
   only live workspace, vault, hook, and MCP-adjacent surfaces.
4. Rewrite `.vaultspec/docs/hooks-guide.md` to match the runtime hook model:
   actual events, shell-only action model, examples, and failure behavior.
5. Reframe `.vaultspec/README.md` as the framework-structure manual rather than
   a public product overview.
6. Correct the package Documentation URL so repository metadata points users to
   the real documentation surface.
7. After the runtime defects above are fixed, add one verified MCP setup example
