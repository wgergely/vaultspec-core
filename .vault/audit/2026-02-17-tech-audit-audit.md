...
1. **Lazy imports everywhere**: Heavy dependencies (torch, sentence-transformers, lancedb, acp, a2a) are imported at call time, not module level. This allows the CLI and vault tools to work without RAG deps.

1. **Config singleton**: `core.config.get_config()` returns a module-level singleton. Overrides create fresh instances (not cached). All configurable values have VAULTSPEC\_\* env var counterparts.

1. **Security-first**: `safe_read_text()` validates paths against workspace root. `_sanitize_filter_value()` escapes SQL injection in LanceDB queries. `_is_vault_path()` enforces read-only boundaries. `_validate_include_dirs()` prevents path traversal.

1. **Dependency injection**: Both ACP bridge and A2A executors accept factory callables for SDK client/options creation, enabling thorough unit testing without real API calls.

1. **Atomic writes**: `atomic_write()` in cli.py uses tmp-file-then-rename to prevent partial writes.

1. **Thread safety**: TaskEngine and LockManager use `threading.Lock` for concurrent access. AsyncIO events bridge async/sync boundaries.

1. **Session management**: The ACP bridge tracks session state (`_SessionState`) enabling load/resume/fork operations, though Claude SDK doesn't persist conversation history across client instances.
...
1. Encode query text with query prefix

1. Execute hybrid search (BM25+ANN) with RRF reranking, fetch 3x requested results

1. Apply graph-aware re-ranking (authority 0.1x min(in_links,10), neighborhood 1.15x, recency 0.02x rank)

1. Return top-k results with snippets

1. Validate agent exists in cache

1. Create TaskEngine task (status: WORKING)

1. Spawn background coroutine:
   c. Call run_subagent() with all parameters
   d. On completion: extract artifacts, truncate summary, complete_task()

1. Return immediately with taskId

1. `spawn_agent_process()` starts the bridge subprocess

1. Client calls `new_session()` with CWD and MCP servers

1. Client sends `prompt()` with TextContentBlock list

1. Bridge calls `sdk_client.query()` and streams SDK events as ACP session updates

1. Client receives AgentMessageChunk, ToolCallStart, ToolCallProgress notifications

1. Client calls `cancel()` to terminate
