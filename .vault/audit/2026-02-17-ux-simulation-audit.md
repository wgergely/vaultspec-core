...
2. **The User Manual is excellent but assumes too much context.** It jumps straight into skills and agents without explaining what `.vault/` is, what artifacts are, or what the expected directory structure looks like after setup.
3. **Typo on line 4:** "developmment" (double m).
4. **Missing "Prerequisites" section.** What tools need to be installed? (Python, CUDA, fd, rg, sd, sg, etc.)
...
2. **`FRAMEWORK.md` is only 4 lines** referencing `.vaultspec/README.md`. It feels like a redirect rather than a standalone document.
3. **`PROJECT.md` is completely empty.** This is the "user-editable" file for project-specific context, but there's no example or guidance on what to put there.
...
2. **`subagent.py` crashes on import** with `ModuleNotFoundError: No module named 'logging_config'`. This completely blocks subagent dispatch via CLI, which is a core feature described throughout the documentation.

3. **No remove/rename/delete CLI commands** for rules, agents, or skills. The framework positions itself as a "managed" system but only provides half of CRUD operations.

### Major

4. **The actual user manual (`.vaultspec/README.md`) is buried** inside a hidden directory. New users reading the top-level `README.md` get a 7-line summary that doesn't explain how to get started.

5. **The framework's own `.vault/` fails its own verification** with 93 errors (naming violations, missing tags, broken links, orphaned documents). This is a bad first impression -- "eat your own dog food."
...
8. **`workflows/` directory is referenced but doesn't exist.** This creates confusion about what workflows are vs. skills.

9. **`PROJECT.md` is empty** with no example content or guidance.

10. **Typo in README.md:** "developmment" (line 4 of `.vaultspec/README.md`).

11. **System prompt parts (`system/`) include Gemini-specific PowerShell instructions** that would be confusing if synced to a non-Windows environment.

12. **No version or changelog** visible to users. The pyproject.toml shows `0.1.0` but this isn't surfaced anywhere in CLI output or docs.
...
3. **Template quality is high.** Self-documenting YAML frontmatter with inline comments, strict tagging rules, and consistent structure across all 8 templates.

4. **`sync-all --dry-run` is confidence-inspiring.** Users can preview exactly what files will be created/modified before committing to the operation.
...
8. **Skills are well-documented** with clear activation instructions, output specifications, and template references.
...
2. Fix `subagent.py` import error.
3. Add `remove` commands for rules, agents, and skills (`cli.py agents remove <name>`).
4. Add a `vaultspec init` or `cli.py init` command that bootstraps `.vaultspec/` and `.vault/` in a new project.
5. Fix the 93 verification errors in the framework's own `.vault/` directory.

### Short-Term (Should-Have)

6. Add `show` commands for individual resources (`cli.py agents show <name>`).
7. Add `rename` commands for resources.
8. Document GPU/CUDA requirements for RAG features in CLI help text.
9. Add a `--version` flag to all CLI tools.
10. Create a `workflows/` directory with at least one example workflow, or remove references to it.
