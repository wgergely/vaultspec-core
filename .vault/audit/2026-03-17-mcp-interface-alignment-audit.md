...
1. **1:1 CLI parity**: same arg names, same semantics where possible
1. **Single-purpose tools**: each tool does one thing
1. **Prod-only**: no dev/build/test commands
1. **Read-heavy surface**: predominantly read/query; writes limited to `vault_add`
1. **No redundancy**: no duplicate tools for the same operation
...
1. **Split** `audit_vault` into `vault_stats` + `vault_doctor`
1. **Remove** `workspace_status` (absorbed by `vault_doctor`)
1. **Split** `list_spec_resources` / `get_spec_resource` into per-domain tools
1. **Add** `vault_graph`, `vault_feature_list`, `spec_system_show`
1. **Align** arg contracts with CLI (named params, same names)
