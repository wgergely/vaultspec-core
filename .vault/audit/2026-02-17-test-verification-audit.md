...
1. **`orchestration/constants.py`** -- no tests (may be trivial constants)

1. **`protocol/a2a/state_map.py`** -- no direct unit tests (indirectly tested via executors)

1. **`rag/query.py`** -- has unit tests in `test_query.py`, but also tested via `test_search_unit.py`

1. **CLI test runner path bug** -- functional test path is `.vaultspec/tests` (wrong), should be `.vaultspec/lib/tests`

1. **`subagent.py` self-bootstrap** -- script cannot find `logging_config` when run directly

1. **No test isolation for `vault.py`** -- the docs CLI has no dedicated tests

1. **Benchmark runner (`bench_rag.py`)** -- exists but not included in any test marker category
...
1. **Centralized constants** -- `tests/constants.py` eliminates scattered path definitions.
1. **Vault snapshot reset** -- `_vault_snapshot_reset()` autouse fixture restores `test-project/.vault/` after tests.
1. **No sys.path hacks in conftest** -- clean `pythonpath` config in pyproject.toml.
1. **Good marker discipline** -- unit, api, search, index, quality markers well-defined.
1. **Strong edge case coverage** -- robustness tests for unicode, SQL injection, code blocks, embedded YAML separators.

### Weaknesses

1. **CLI test runner broken** -- wrong path in `test_run()`.
1. **Timeout defaults too low** -- RAG functional tests need 300s, not the 120s default. The initial full run hung because `test_index_incremental` (full corpus re-index) exceeded the default timeout.
1. **Missing skip decorators on API-dependent tests** -- A2A e2e tests should be conditionally skipped.
1. **`subagent.py` import failure** -- the script assumes `logging_config` is on sys.path, but doesn't bootstrap it.
1. **No CI configuration visible** -- no `.github/workflows/` or similar CI setup found. Test automation relies on manual invocation.
...
1. **Fix `subagent.py` import**: Add `_paths.py` bootstrap or make `logging_config` importable via proper package installation.
1. **Add skip markers to A2A e2e tests**: Use `@pytest.mark.claude` / `@pytest.mark.gemini` with `skipIf` to avoid false failures.
1. **Increase RAG test timeouts**: Set `test_index_incremental` to 300s or mark as `@pytest.mark.quality`.
1. **Add `vault.py` tests**: The docs CLI has no test coverage.
1. **Add CI pipeline**: No automated test runs found. GitHub Actions would catch regressions.
