...
from ...config import LayoutMode, WorkspaceError, resolve_workspace
from ...core import collect_rules, init_paths
...
def test_cli_help():
    result = subprocess.run(
        [sys.executable, "-m", "vaultspec.subagent_cli", "--help"],
        capture_output=True,
        text=True,
    )
...
def test_cli_list_agents():
    result = subprocess.run(
        [sys.executable, "-m", "vaultspec.subagent_cli", "list"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
class TestContentDirCLI:
    ...
    def test_init_paths_uses_content_root(self, tmp_path: Path) -> None:
        ...
        from ...config import LayoutMode, WorkspaceLayout
        ...
        assert output == _types.ROOT_DIR
        assert content / "rules" / "rules" == _types.RULES_SRC_DIR
        assert content / "rules" / "agents" == _types.AGENTS_SRC_DIR
        assert content / "rules" / "skills" == _types.SKILLS_SRC_DIR
