...
from ...core.sync import sync_skills
...
class TestSyncSkills:
    def _make_skill_sources(
        ...
        sources = {}
        for name in names:
            # Create source as directory with SKILL.md to match new structure
            d = root / name
...
class TestSystemSync:
    def test_generates_from_parts(self):
        ...
        assert content.startswith(CONFIG_HEADER)
...
class TestEndToEnd:
    def test_full_sync_cycle(self):
        ...
        lint_skill_dir = (
            TEST_PROJECT / ".vaultspec" / "rules" / "skills" / "vaultspec-lint"
        )
