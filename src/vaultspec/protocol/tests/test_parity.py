...
class TestReadonlySandboxBothProviders:
    """Gemini communicates sandbox via VAULTSPEC_AGENT_MODE env; Claude does not change
    args."""

    def test_gemini_readonly_sets_agent_mode_env(self, gemini):
        spec = _gemini_spec(gemini, mode="read-only")
        assert spec.env["VAULTSPEC_AGENT_MODE"] == "read-only"

    def test_gemini_readwrite_sets_agent_mode_env(self, gemini):
