...
class VaultSpecConfig:
    ...
    Attributes:
        ...
        content_dir: Content source directory override.
        agent_mode: Agent permission mode (``"read-write"`` or ``"read-only"``).
        system_prompt: Custom system prompt for agent sessions.
        max_turns: Maximum conversation turns per agent session.
        budget_usd: Budget cap in USD for agent sessions.
        allowed_tools: List of tool names the agent is permitted to use.
        disallowed_tools: List of tool names the agent is not permitted to use.
        effort: Effort level hint for agent sessions.
        output_format: Output format for agent responses.
        fallback_model: Fallback model identifier for agent sessions.
        include_dirs: Directories to include when building the agent context.
        mcp_root_dir: Root directory for the MCP server.
        mcp_port: Port for the MCP server.
        mcp_host: Host address for the MCP server.
        mcp_ttl_seconds: Task TTL in seconds for the MCP server.
        a2a_default_port: Default port for A2A agent cards.
        a2a_host: Default host for A2A agent cards.
        docs_dir: Documentation vault directory name.
        framework_dir: Framework directory name.
        lance_dir: LanceDB vector store directory name.
        index_metadata_file: Index metadata filename within the lance directory.
        logs_dir: Log subdirectory name within the documentation vault.
        claude_dir: Claude tool directory name.
        gemini_dir: Gemini tool directory name.
        agent_dir: Agent tool directory name.
        task_engine_ttl_seconds: Task engine TTL in seconds.
        log_retention_days: Number of days to retain session log files.
        graph_ttl_seconds: Graph cache TTL in seconds for search re-ranking.
        embedding_batch_size: Batch size for GPU embedding inference.
        max_embed_chars: Max characters per document for embedding truncation.
        embedding_model: Sentence-transformer model name for embeddings.
        embedding_dimension: Embedding vector dimension.
        rag_enabled: Whether RAG features are enabled.
        io_buffer_size: I/O read buffer size in bytes.
        terminal_output_limit: Terminal output byte limit for subprocess capture.
        mcp_poll_interval: Agent file polling interval in seconds for MCP server.
        editor: Default editor command for creating rules/agents/skills.
    """
...
CONFIG_REGISTRY: list[ConfigVariable] = [
    ...
    ConfigVariable(
        ...
        default=True,
        description=(
            "Enable/disable RAG features. Set to false in GPU-less environments."
        ),
    ),
    # -- Server ----------------------------------------------------------------
    ConfigVariable(
