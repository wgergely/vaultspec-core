...
class WorkspaceLayout:
    ...
    Attributes:
        content_root: Directory containing rules, agents, and skills source files.
        output_root: Root directory for tool-specific output (CLAUDE.md, etc.).
        vault_root: Path to the ``.vault/`` documentation directory.
        framework_root: Path to the ``.vaultspec/`` framework directory.
...
def _validate(layout: WorkspaceLayout) -> None:
    ...
    Raises:
        WorkspaceError: If ``content_root`` is not an existing directory or
            ``output_root``'s parent does not exist.
    """
...
def resolve_workspace(
    *,
    ...
    Args:
        root_override: Explicit output root (``--root`` / ``VAULTSPEC_ROOT_DIR``).
        content_override: Explicit content root
            (``--content-dir`` / ``VAULTSPEC_CONTENT_DIR``).
