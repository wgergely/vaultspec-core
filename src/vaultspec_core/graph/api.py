"""Build and query the vault document relationship graph.

This module turns `vaultcore` scanning, metadata parsing, and wiki-link
extraction into a queryable graph of `.vault/` documents and their
relationships. It is the analysis layer for link structure, hotspots, feature
rankings, orphaned documents, and broken links.

Usage:
    Instantiate `VaultGraph(root_dir)` for a vault root, then call the query
    methods to inspect document relationships and graph-derived summaries.
"""
