"""Verify vault structure, metadata integrity, and feature-level conformance.

This module validates `.vault/` layout and document metadata against the typed
vault model, reports structural violations, derives feature coverage, and
supports limited repair of common integrity issues. It is the primary
conformance layer for checking whether vault content matches project rules.

Usage:
    Use `get_malformed(root_dir)` for full-vault structural checks,
    `verify_file(path)` for a single document, and `fix_violations(root_dir)`
    only when applying repairs is part of the intended workflow.
"""
